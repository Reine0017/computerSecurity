...
void print_version(char* cmd)
{
        ...
        snprintf(buffer, sizeof(buffer), "Submission program version 0.1 (%s)\n"
, cmd);
...
//Checks for substrings in the program that have been connected to troublesome s
ubmissions
static
int check_for_viruses(char * filename) {
        ...
        while (c != EOF) {
                ...
                if(regcomp(&regex, "bin/sh", 0)){
                        fprintf(stderr, "Could not compile known virus signature
s\n");
                        return 1;
               }
                ...
                if(!reti){
                        printf("Alert! Detected possibly malicious submission! T
erminating.\n");
                        return  1;
                }
        }
...
void print_usage(char* cmd)
{
        ...
        snprintf(txt, 640, "Syntax: %s <path> [log message]\n"
                "-s, --submitted        Show your submitted files\n"
                "-v, --version          Show version\n"
                "-h, --help             Show this usage message\n", cmd);

        for (i = 2; i >= 0; i--) {
                snprintf(txt2, 64, "Exiting program... (%d)", i);
        }
...
static
int show_confirmation(char* msg) {
        char *subdir_name;
        subdir_name = get_submit_dir();
        printf("%s\n", msg);
